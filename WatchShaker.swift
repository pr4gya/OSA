...
//
//  Created by Pragya Prakash on 8/01/2021
//
import Foundation
import CoreMotion
...
public class WatchShaker : NSObject
{
    ...
    public func start(delay accelerometerUpdateInterval:Double = 0.02)
    {
        ...
        motionManager.startAccelerometerUpdates(to: motionQueue) { (accelerometerData, err) -> Void in
            ...
            guard let data = accelerometerData, err == nil else
            {
                ...
                self.startWatchShakerUpdates?(nil, nil,nil ,err ?? e)
                return
            }
            ...
            let maxValuexy = fabs(valueX) > fabs(valueY) ? fabs(valueX) : fabs(valueY)
            let maxValue = maxValuexy > fabs(valueZ) ? maxValuexy : fabs(valueZ)
    ...
    private func didShake() {
        ...
        self.startWatchShakerUpdates?(self.sensibility, self.coordinates, .direction(coordinates?.x ?? 0, coordinates?.y ?? 0), nil)
    }
}
